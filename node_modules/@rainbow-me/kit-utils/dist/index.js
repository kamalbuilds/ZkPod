...
const setupProvider = (web3ProviderInit) => {
    return function getLibrary(provider) {
        const library = new Web3Provider(provider);
        library.pollingInterval = 12000;
        web3ProviderInit === null || web3ProviderInit === void 0 ? void 0 : web3ProviderInit(library);
        return library;
    };
};
const withWeb3React = (Component, web3ProviderInit) => {
...
const parseSendReturn = (sendReturn) => sendReturn.result || sendReturn;
...
(function (ChainId) {
    ...
    ChainId[ChainId["ECO"] = 128] = "ECO";
    ChainId[ChainId["FANTOM"] = 250] = "FANTOM";
    ChainId[ChainId["AOX"] = 200] = "AOX";
    ...
    ChainId[ChainId["EDGE"] = 2021] = "EDGE";
    ChainId[ChainId["NEAR"] = 1313161554] = "NEAR";
...
const chainIdToAlias = (id) => { var _a; return ((_a = chains.find((chain) => chain.chainId === id)) === null || _a === void 0 ? void 0 : _a.aliases[0]) || 'ethereum'; };
const chainIDToToken = (id) => { var _a; return ((_a = chains.find((chain) => chain.chainId === id)) === null || _a === void 0 ? void 0 : _a.nativeCurrency.symbol) || 'ETH'; };
const chainIDToExplorer = (id) => {
    var _a, _b;
    return ((_b = (_a = chains.find((chain) => chain.chainId === (!id || id === 0 ? 1 : id))) === null || _a === void 0 ? void 0 : _a.explorers) === null || _b === void 0 ? void 0 : _b[0]) || {
        name: 'etherscan',
...
const toSignificant = (b) => {
    const floatBal = parseFloat(formatEther(b));
    return floatBal > 9999 ? floatBal.toFixed(0) : floatBal.toPrecision(4);
};

const guessTitle = ({ data, from, to, chainId = 1, value }) => {
    if (data === '0x') {
        if (from === to)
            return `Cancel transaction`;
        else
            return `Transfer ${toSignificant(value)} ${chainIDToToken(chainId)} from ${shortenAddress(from)} to ${shortenAddress(to)}`;
    }
    else
        return `Contract call`;
};

const switchNetwork = async (provider, chain) => {
    const obj = {
...
const logsFetcher = async ({ provider, address, options = {} }) => {
    return await provider.getLogs({ address, ...options });
};
const etherscanFetcher = async ({ provider, address, options = {} }) => {
...
const popularEmojis = avatars.map((avatar) => avatar.emoji);
const emojiColorIndexes = avatars.map((avatar) => avatar.colorIndex);
...
const resolveAddress = async ({ addr, provider }) => {
